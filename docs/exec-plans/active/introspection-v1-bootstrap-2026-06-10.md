...
**Status:** Complete on 2026-06-12. `claude-toolkit` branch `introspection/wi18-deferral-rule` is pushed; its net diff against `origin/main` changes only `GLOBAL_CLAUDE.md` and `GLOBAL_AGENTS.md`, replacing tracker-first/phase-stub wording with repo-tracking-system wording, adding explicit exclusions for brainstorms, rejected ideas, same-session tasks, vague maybes, and owner-doc duplicates, and narrowing session-finish audit to accepted unresolved deferrals/action items. Commit `05c8460` made the global-instruction update; follow-up commit `bbfe8e5` reverts unrelated `.codex/config.toml` drift that was pulled in by a toolkit config-sync side effect, leaving the branch scope clean. Verification passed: obsolete wording search, cross-file deferral-block equality check, and `pnpm check`. `toolkit-lint` still fails on pre-existing skill-size/artifact findings unrelated to `GLOBAL_CLAUDE.md` or `GLOBAL_AGENTS.md`; review and cleanup gates accepted that as a verification exception and did not accept ownership of those existing lint failures as WI-18 follow-up work.
...
**Status:** Repo-side release prep complete on 2026-06-12; full WI-19 remains blocked on external/manual publish steps. Commit `ea4f97b` (`feat: add release publishing prep`) added the Changesets/OIDC/provenance release workflow, release/version scripts, fail-closed `release:prepare` gate (`pnpm pack:dry-run && pnpm pack:smoke`), first-release changeset for the main package plus three platform packages, and `docs/references/release-readiness.md`. Verification passed: release workflow/script assertions, `pnpm release:prepare` with native and fallback packed-install smokes, repo prose, and full `pnpm check` with 169 tests. Review and refactor/cleanup gates reported no blockers or should-fix issues. Remaining WI-19 blockers are external/future: MP must configure npm Trusted Publishing bindings or perform a one-time first-publish bootstrap if npm requires package records first; actual publish has not happened; backpressure still needs the published strict-catalog pin plus release-age handling after publish; and the TD-CARD-037 branch-protection ruleset must be re-verified before trusting the auto-publish path.
